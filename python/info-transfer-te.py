...
for run in runs:
    ...
    values = {}
    path = os.path.join(run, "plots", "data", "info-transfer-te-{0}.txt".format(args.stage))
    with open(path) as f:
        for line in f:
            if line.startswith("#"):
                continue
...
axes.set_ylabel("Information transfer")
axes.set_ylim(bottom = max(0, axes.get_ylim()[0]))
figure.tight_layout()
figure.savefig("info-transfer-te-{0}.pdf".format(args.stage))
