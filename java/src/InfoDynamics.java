...
public class InfoDynamics {
    ...
    private static enum Metric {
        ACTIVE_INFO_STORAGE("S"),
        APPARENT_TRANSFER_ENTROPY("T"),
        SEPARABLE_INFO("M");
        ...
        public String getId(String subset) {
            ...
            return String.format("%s[%s]", id, subset);
        }
    }
    ...
    private static Result getSeparableInfo(Predicate<Double> predicate, String subset) {
        Result result = new Result();
        for (int neuronIndex : ensemble.getBrain().getNeuronIndices(Brain.Layer.PROCESSING)) {
